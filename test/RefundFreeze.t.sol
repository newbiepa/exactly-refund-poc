...
contract RefundFreezeTest is Test {
    ...
    function testPOC_CollectDebitBurnsWithoutCorrespondingMints() public {
        ...
        if (balanceInitial > balanceAfterBurnC) {
            ...
            assertLt(
                balanceAfterBurnC, balanceInitial, "PoC VERIFICADO: Collect Debit burns resultaron en perdida neta"
            );
        }
        ...
        if (permanentLoss > 0.5e6) {
            // Al menos $0.50 USDC perdida permanente
    ...
    function testPOC_SystematicBehaviorAcrossMultipleBurns() public {
        ...
        if (currentBalance == afterBurnC) {
            ...
            assertLt(currentBalance, initialBalance, "BUG SISTEMATICO: Perdida neta permanente confirmada");
        }
    ...
    function testPOC_FalseUIRefundDisplay() public {
        ...
        assertTrue(balanceCurrent < balanceInitial, "BUG UI DEMOSTRADO: UI muestra refund falso - fondos siguen frozen");
    }
    ...
    function _formatBalance(uint256 balance) private view returns (string memory) {
        ...
        return string(abi.encodePacked(vm.toString(wholePart), ".", _padZeros(fractionalPart, 4)));
    }
